...
class VLM_Yibu:
    ...
    def _post_process_model_for_path_intents(self) -> str:
        post_process = self.configs.post_process
        model = (
            getattr(post_process, "vlm_model_name_for_path_intents", None)
            or getattr(post_process, "vlm_model_name_for_path_conditioned_goals", None)
            or getattr(post_process, "vlm_model_name_for_transition_info", None)
            or getattr(post_process, "vlm_model_name", None)
        )
        if not model:
            raise ValueError(
                "post_process.vlm_model_name_for_path_intents, "
                "post_process.vlm_model_name_for_path_conditioned_goals, "
                "post_process.vlm_model_name_for_transition_info, or post_process.vlm_model_name must be set"
            )
        return str(model).strip()
    ...
    def get_node_user_intents(self, node_info, path_intents, out_edges):
        ...
        model_name = self.configs.post_process.vlm_model_name_for_node_user_intents
        max_attempts = int(getattr(self.configs.post_process, "request_retries", 5) or 5)
        last_error = None
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                result = parse_json_from_model_response(text)
                ...
                if attempt < max_attempts:
                    ...
                    continue
                raise ValueError(
                    "Failed to parse node-user-intents VLM response.\n"
                    f"model={model_name}, attempt={attempt}/{max_attempts}\n"
                    f"raw_content={response_content!r}\n"
                    f"raw_text={text!r}\n"
                    f"error={exc}"
            ...
            if not isinstance(user_intents, list):
                user_intents = [user_intents] if user_intents else []
    ...
    def get_transition_info(
        ...
        model_name = self.configs.post_process.vlm_model_name_for_transition_info
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if attempt < max_attempts:
                    ...
                    continue
                raise ValueError(
                    "Failed to parse transition-info VLM response.\n"
                    f"model={model_name}, attempt={attempt}/{max_attempts}\n"
                    f"raw_content={response_content!r}\n"
                    f"raw_text={text!r}\n"
                    f"error={exc}"
    ...
    def get_path_intents(self, trajectory):
        ...
        model_name = self.configs.post_process.vlm_model_name_for_path_intents
        max_attempts = int(getattr(self.configs.post_process, "request_retries", 5) or 5)
        last_error = None
        response_content = None
        text = ""
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if attempt < max_attempts:
                    ...
                    continue
                raise ValueError(
                    "Failed to parse path-intents VLM response.\n"
                    f"model={model_name}, attempt={attempt}/{max_attempts}\n"
                    f"raw_content={response_content!r}\n"
                    f"raw_text={text!r}\n"
                    f"error={exc}"
            ...
            path_reliability = result.get("path_reliability", {})
            if not isinstance(path_reliability, dict):
                path_reliability = {}

            score = int(path_reliability.get("score", 0))
            score = max(0, min(5, score))

            path_user_goals = result.get("path_user_goals", [])
            if not isinstance(path_user_goals, list):
                path_user_goals = [path_user_goals] if path_user_goals else []

            return {
                "path_user_goals": path_user_goals,
                "path_reliability": {
                    "score": score,
                    "reason": path_reliability.get("reason", ""),
                },
            }
    ...
    """.strip()
        ...
        model_name = getattr(
            ...
            "vlm_model_name_for_top_paths",
            None,
        ) or self.configs.post_process.vlm_model_name_for_path_intents
        max_attempts = int(getattr(self.configs.post_process, "request_retries", 5) or 5)
        last_error = None
        response_content = None
        text = ""
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if attempt < max_attempts:
                    ...
                    continue
                raise ValueError(
                    "Failed to parse top-paths VLM response.\n"
                    f"model={model_name}, attempt={attempt}/{max_attempts}\n"
                    f"raw_content={response_content!r}\n"
                    f"raw_text={text!r}\n"
                    f"error={exc}"
            ...
            selected = result.get("selected_path_ids", [])
    ...
    Input contains:
    - target_page
    - paths; each path has num_pages (L), num_transitions (L-1), ordered pages, and transitions
    ...
    - Produce exactly one transition_hint per input transition (one per hop), never one per alternative_action.
    ...
    """.strip()
        ...
        model_name = self.configs.post_process.vlm_model_name_for_navigation_plan
        max_attempts = int(getattr(self.configs.post_process, "request_retries", 5) or 5)
        last_error = None
        response_content = None
        text = ""
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                return parse_json_from_model_response(text)
                ...
                if attempt < max_attempts:
                    ...
                    continue
                raise ValueError(
                    "Failed to parse navigation-plan VLM response.\n"
                    f"model={model_name}, attempt={attempt}/{max_attempts}\n"
                    f"raw_content={response_content!r}\n"
                    f"raw_text={text!r}\n"
                    f"error={exc}"
